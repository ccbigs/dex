<template>
  <div class="title">
    ...
    <div class="img_dianzan">
      <img v-if="dianzan === 0" src="/static/images/dianzan.png" alt="" @click="approve">
      <img v-else src="/static/images/dianzaned.png" alt="" @click="approve">
    </div>
...
export default {
  ...
  methods:{
    ...
    async getInformationDetail() {
      const res=await this.axios.post('http://localhost:8680/dex/info/getInformationDetailByBusman',this.$qs.stringify({
    ...
    async increaseArticleViews() {
      const res=await this.axios.post('http://localhost:8680/dex/info/increaseArticleViews',this.$qs.stringify({
        info_id:this.$route.query.info_id
      }))
    },
    /*返回上级页面*/
    ...
    approve(){
      if (this.dianzan === 0){
        this.dianzan=1
        this.axios.post('http://localhost:8680/dex/info/readersLikeArticles',this.$qs.stringify({
        ...
        this.dianzan=0;
        this.axios.post('http://localhost:8680/dex/info/readersCancelFavoriteArticles',this.$qs.stringify({
          info_id:this.information.id
        })).then((res)=>{
          if (res.data.code === 200){
            this.getInformationDetail()

    ...
    <div class="order-manager">
      <el-table
        ...
        <el-table-column
            ...
            width="120">
          <template slot-scope="scope">
            <el-tag size="medium" type="success" v-if="scope.row.carousel_state === '启用'">{{ scope.row.carousel_state }}</el-tag>
            <el-tag size="medium" type="danger" v-if="scope.row.carousel_state === '停用'">{{ scope.row.carousel_state }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column
            prop="createDate"
...
export default {
  ...
  methods:{
    async getDidBuyCarousel(){
      const res=await this.axios.post('quotient/getDidBuyCarousel',this.$qs.stringify({
